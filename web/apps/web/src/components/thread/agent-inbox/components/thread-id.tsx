import { Check, CopySharp } from "pixelarticons/react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
    ...
    >
      ...
      <AnimatePresence mode="wait" initial={false}>
        {copied ? (
          ...
          >
            <Check className="text-green-500 max-w-3 w-3 max-h-3 h-3" />
          </motion.div>
        ) : (
          <motion.div
            key="copy"
            ...
            transition={{ duration: 0.15 }}
          >
            <CopySharp className="text-gray-500 max-w-3 w-3 max-h-3 h-3" />
          </motion.div>
        )}
      </AnimatePresence>
    </TooltipIconButton>
